# 👋 안녕하세요, 저는 지송(Jisong)입니다!

저의 깃허브에 오신 것을 환영합니다! 아래는 제가 현재 진행 중인 주요 프로젝트들을 생태계별로 분류해 둔 포트폴리오입니다.

## ☁️ 지송 클라우드 생태계 (Jisong Cloud Ecosystem)

*개인 클라우드 인프라 및 웹 포트폴리오*

* 🌐 **[jisong.dev](https://github.com/jsbang01357/jisong.dev)** - 개인 포트폴리오 웹사이트.
* ☁️ **[JisongCloud](https://github.com/jsbang01357/jisong_cloud)** - Google Cloud Storage를 활용한 비밀번호 보호 기반의 개인 클라우드 스토리지 애플리케이션.

## 🧬 의료 공학 및 AI (Medical Engineering)

*의료 데이터 파이프라인 및 AI 보조 임상의사결정지원시스템(CDSS) 프로토타입*

### 🖼️ Frame Series

* 🩺 **[DiaFrame](https://github.com/Jisong-Medical-Engineering/DiaFrame)** - Vertex AI를 활용한 당뇨병 약제 추천 및 검증을 위한 내분비내과 CDSS 프로토타입.
* 🧠 **[NeuroFrame](https://github.com/Jisong-Medical-Engineering/NeuroFrame)** -  Shift-aware Cognitive Planning Engine
* 👩 **[EstroFrame](https://github.com/Jisong-Medical-Engineering/EstroFrame)** - 트랜스여성 GAHT CDSS
* 👨 **[AndroFrame](https://github.com/Jisong-Medical-Engineering/AndroFrame)** - 트랜스남성 GAHT CDSS

### Data Pipline

* **[CleanText](https://github.com/Jisong-Medical-Engineering/Clean_Text)** - 로컬 브라우저 환경에서 처리되는 안전한 EMR 텍스트 파싱 및 CSV 출력 툴.
* **[CleanEMR](https://github.com/Jisong-Medical-Engineering/Clean_EMR)** - EMR 데이터 SQLite 구조화.

### 🧪 Others

* 🍇 **[VoiceGrape](https://github.com/Jisong-Medical-Engineering/VoiceGrape)** - 이비인후과 음성 처리 및 분석 툴.

## 🛠️ 사이드 프로젝트 (Side Projects)

* 🐈 **[Choonsik-Workspace](https://github.com/jsbang01357/Choonsik-Workspace)** - 개인용 Openclaw 에이전트
* **[🌌 Obsidian PKM](https://github.com/jsbang01357/Obsidian_jsbang)** - 개인 지식 구조화 프로젝트
* 🎵 **[YT_Playlist](https://github.com/jsbang01357/YT_Playlist)** - 유튜브 플레이리스트 관리 유틸리티.

---

*의료 공학(Medical Engineering) 프로젝트 개발이나 협업에 관심이 있으시다면, 언제든 편하게 연락해 주세요!*
📧 **Email:** jsbang01357@gmail.com
